...
# 時刻付き期限
todome add プレゼン準備 +仕事 due:2026-03-01T18:00:00
```
...
todome list -s created         # 作成日時順（新しい順）
todome list -s due             # 期限順（近い順、期限なしは末尾）
todome list -s completed       # 完了日時順（新しい順）
...
```
(A) 2026-02-23T14:30:00 企画書を書く +仕事 @PC due:2026-03-01
x 2026-02-24T10:00:00 2026-02-20T09:00:00 牛乳を買う +買い物 note:やりきった！
2026-02-23T08:00:00 レポートを提出する
```
...
| 完了マーク | `x ` | 行頭に付くと完了タスク |
| 優先度 | `(A)` | A〜Z で優先度を表す |
| 日時 | `2026-02-23T14:30:00` | 作成日時（完了時は完了日時 + 作成日時） |
| 日付（互換） | `2026-02-23` | 従来の日付のみ形式も読み込み可能 |
| プロジェクト | `+仕事` | `+` に続くタグ |
| コンテキスト | `@PC` | `@` に続くタグ |
| 期限 | `due:2026-03-01` | 日付のみの期限 |
| 期限（時刻付き） | `due:2026-03-01T18:00:00` | 時刻付きの期限 |
| 完了メモ | `note:やった！` | 完了時のメッセージ |
